      ...
      try {
        ...
        if (window.confirm('This will replace all current data. Continue?')) {
          ...
          try {
            ...
            alert('Error importing to database. Imported to localStorage only.')
    ...
    if (window.confirm('This will delete ALL data from the database. This cannot be undone. Are you sure?')) {
      if (window.confirm('Final confirmation: Delete everything?')) {
        try {
          ...
          setWorkers([])
          localStorage.clear()
          alert('All data cleared from database and localStorage')
        } catch (error) {
          console.error('Error clearing data:', error)
          alert('Error clearing database. Cleared localStorage only.')
          setJobs([])
          setCustomers([])
          setWorkers([])
          localStorage.clear()
